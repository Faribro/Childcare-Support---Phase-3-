...
function normalizeSubmissionData(raw: any, submissionId: string, fallbackVersion: number = 1): any {
  ...
  const normalized: any = {
    ...
    educationExpenses: {
      ...
      tuitionFees: Number(raw['57\nPrivate Tuition Fee'] || raw.tuition_fees || raw.tuitionFees || raw.educationExpenses?.tuitionFees || 0),
      books: Number(raw['58\nSchool Books'] || raw.books || raw.educationExpenses?.books || 0),
      stationery: Number(raw['59\nSchool Stationery'] || raw.stationery || raw.educationExpenses?.stationery || 0),
      uniform: Number(raw['60\nSchool Uniform'] || raw.uniform || raw.educationExpenses?.uniform || 0),
      transport: Number(raw['61\nSchool Transport'] || raw.transport || raw.educationExpenses?.transport || 0),
      otherExpenses: Number(raw['62\nSchool Other Expenses'] || raw.other_expenses || raw.otherExpenses || raw.educationExpenses?.otherExpenses || 0),
      remarks: raw['66\nRemarks (If Any)'] || raw.remarks || raw.clinical_notes || raw.educationExpenses?.remarks || '',
