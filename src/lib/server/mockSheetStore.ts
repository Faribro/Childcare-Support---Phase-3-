...
export const MockSheetStore = {
  ...
  createRecord(payload: CompleteSubmissionPayload, idempotencyKey: string, requestId: string): CreateRecordResult {
    ...
    const newRecord: StoredSheetRecord = {
      ...
      account_holder_name: payload.bankDetails?.accountHolderName || payload.demographics.caregiverName,
      bank_account_number: payload.bankDetails?.accountNumber || '123456789012',
      ifsc_code: payload.bankDetails?.ifscCode || 'SBIN0001234',
      bank_name: payload.bankDetails?.bankName || 'State Bank of India',
      branch_name: payload.bankDetails?.branchName || 'Main',
      passbook_photo_captured: payload.bankDetails?.passbookPhotoCaptured ?? true,
